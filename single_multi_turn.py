...
def initialize_llm(model_id):
    """
    Initializes the HuggingFace LLM with specified configurations.
    """
    # Load tokenizer
...
agentic_template = PromptTemplate(
    input_variables=["user_question", "relevant_cols", "ds_id"],
    template=AGENTIC_INITIAL_PROMPT
)

# Initialize Memory for the Agentic LLM
agentic_memory = ConversationBufferMemory(memory_key="chat_history")

# Create the Agentic LLM Chain
def create_agentic_chain(llm):
    """
    Creates an LLMChain with the defined prompt and memory.
    """
    return LLMChain(
        llm=llm,
        prompt=agentic_template,
...
def agentic_system(user_question, relevant_cols, ds_id, model_id=MODEL_ID):
    """
    ...
    while turn_count < MAX_TURNS:
        ...
        try:
            agentic_response = agentic_chain.run(**agentic_input)
        except Exception as e:
...
if __name__ == "__main__":
    ...
    final_answer, steps_log = agentic_system(user_question, relevant_cols, ds_id, MODEL_ID)
